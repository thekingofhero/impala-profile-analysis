...
def collect_statistics(profile_path, output_file_path, output_dir_path="."):
			...
			if stripped_line.startswith("Estimated Per-Host Requirements:"):
				...
				if key != '' and key in func_map:
					...
					if key == 'UnpinTime':
						...
						# print(curr_id, non_child_time/hash_join_time, build_phase_ratio + probe_phase_ratio)
	...
	# end with
	statistics_file = os.path.split(profile_path)[-1].replace('sql.log','statistic')
#	with open(os.path.join(output_file_path,statistics_file),'w') as fp:
	for key in build_rows_per_ns_hashjoin_node.keys():
		print('HASHJOIN_BUILD_COST,{0},{1}'.format(key, '{0:.8f}'.format(average(build_rows_per_ns_hashjoin_node[key]))))

	for key in probe_rows_per_ns_hashjoin_node.keys():
		print('HASHJOIN_PROBE_COST,{0},{1}'.format(key, '{0:.8f}'.format(average(probe_rows_per_ns_hashjoin_node[key]))))

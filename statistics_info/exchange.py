...
def collect_statistics(profile_path, output_file_path, file_p,output_dir_path="."):
	...
	for key in convert_cost_bytes_per_ns_per_node_map.keys():
		print('EXCH_ROW_BATCH_CONVERT_COST,{0},{1}'.format(key, average(convert_cost_bytes_per_ns_per_node_map[key])),file = file_p)	
	for key in derialize_cost_bytes_per_ns_per_node_map.keys():
		print('EXCH_DESER_COST,{0},{1}'.format(key, average(derialize_cost_bytes_per_ns_per_node_map[key])),file = file_p)
...
if __name__ == '__main__':
	...
	profile = r'..\log_dir\q19.sql.log'
	output = r'..\output_dir\q19'
	sys.argv = ['exchange.py', profile]

	if len(sys.argv) < 2 or not os.path.exists(sys.argv[1]):
		print("usage: exchange.py <impala profile file path or directory>")
		sys.exit()
	collect_statistics( os.path.normpath(profile), os.path.normpath(output))
